	...
	<view class="index1_content">
		...
		<view class="index1_content_top" :style="{background:'#FF7104'}" id="index1_content_top">
			<picker :value="value" mode="multiSelector" @change="picker" @columnchange="columnPicker"
				:range="multiArray" class="index1_content_top_l">
		...
		<!---->
		<scroll-view scroll-y="true" class="index1_content_body" :style="'height:'+height+'px;'"
			@scrolltolower="scrolltolower">
			<!--导航菜单-->
			<view class="index1_content_sort">
				<view class="index1_content_sort_block" v-for="(item,index) in sortList"
					@tap="toList(item.id,item.name)" :key='index'>
					<image :src="item.pic_url?item.pic_url:host+'/images/shop/noneimg.png'"
						class="index1_content_sort_block_icon" mode="widthFix"></image>
			...
			<view class="index1_content_xs">
				<view class="index1_content_xs_block" v-for="(item,index) in dhList"
					:class="index==dhIndex?'xs_block_active':''" @tap="sliderBtn(index)" :key='index'>
			...
			<view class="index1_content_shop">
				...
				<view class="index1_content_shop_block" v-for="(item,index) in shop_list" @tap="toDetail(item.id)"
					:key='index'>
					...
					<view class="index1_content_shop_block_r">
						...
						<view class="index1_content_shop_block_r_cad">付100送{{item.give_value}}购物券</view>
						<view class="index1_content_shop_block_r_m">
							...
							<view class="index1_content_shop_block_r_m_aver" v-if="item.store.average_spend">
								{{item.store.average_spend}}/人</view>
							<view class="index1_content_shop_block_r_m_dist" v-if="item.distance_format">
								{{item.distance_format}}</view>
						</view>
						<view class="index1_content_shop_block_r_m1">
							<view class="index1_content_shop_block_r_m1_l" v-if="item.store.main_product">
								{{item.store.main_product}}</view>
					...
					<image :src="img_url+'dao_location.png'" mode="" class="locatins_logo"
						@click.stop="location(item.store.latitude,item.store.longitude,item.store.address)"></image>
	...
	export default {
		...
		data() {
			return {
				...
				ms_list: [{
						"name": "化妆品",
						"img": "https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=2729596015,2929996475&fm=15&gp=0.jpg",
						"url": ""
					},
					{
						"name": "化妆品",
				...
				// lat:'',//纬度
				// lnt:'',//经度
				lat: uni.getStorageSync('x-latitude'), //纬度
				lnt: uni.getStorageSync('x-longitude'), //经度
				page_count: "", //页面总数
							...
							success: function(result) {
								if (result.confirm) {
									...
									uni.setStorageSync('x-latitude', uni.getStorageSync(
										'x-latitude-new'))
									that.lat = uni.getStorageSync('x-latitude-new') //纬度
									that.lnt = uni.getStorageSync('x-longitude-new') //经度
									that.getData()
...
<style>
	...
	.index1_content_shop_block_r_m_star {
		flex: 1;
		display: flex;
		align-items: center;
	}
	...
	.index1_content_shop_block_r_m1 {
		display: flex;
		align-items: center;
		font-size: 11px;
		font-weight: bold;
	...
	.index1_content_shop_block_r_m1_l {
		flex: 1;
	}

	.index1_content_shop_block_r_m2 {
		margin: 8px auto;
		font-size: 11px;
		font-weight: bold;
		color: #787878;
	...
	.index1_content_shop_block_r_m3 {
		display: flex;
		font-size: 11px;
		color: #787878;
		align-items: center;
	...
	.index1_content_shop_block_r_m3_l {
		width: 11px;
		height: 11px;
		background: #45BCAF;
		text-align: center;
	...
	.index1_content_shop_block_r_m4 {
		display: flex;
		font-size: 11px;
		color: #787878;
		align-items: center;
	...
	.index1_content_shop_block_r_m4_l {
		width: 11px;
		height: 11px;
		background: #DC9106;
		text-align: center;
	...
	.index1_content_shop_block_r_m3_r {
		font-weight: bold;
		margin-left: 4px;
	}

	.index1_content_shop_block_r_m4_r {
		font-weight: bold;
		margin-left: 4px;
	}
