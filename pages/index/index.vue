	...
	<view class="root">
		...
		<block v-for="(item, index) in indexData" :key="index">
			...
			<nav-icon
				...
				class="navIcon_style"
				:list="item.data.navs"
				:textColor="item.data.color"
				:rowNums="item.data.rows"
				:background="item.data.background"
				:listNums="item.data.columns"
			></nav-icon>
			<!-- 导航按钮 -->
			<!-- 轮播图 -->
			...
			<view class="content">
				...
				<!-- 专题 -->
				<view class="thematicBox" v-if="item.id == 'topic'"><thematic :topicData="item.data" @click="topicNavTo"></thematic></view>
				<!-- 专题 -->

				<!-- 团购 -->
				<view v-if="item.id == 'group'" :style="{ background: g_listStyle == 3 ? '' : '#ffffff' }">
					<group-buy
						:listStyle="g_listStyle"
						:productNameBool="g_productNameBool"
						:priceBool="g_priceBool"
						:buyBtnBool="g_buyBtnBool"
						:subscriptStyle="g_subscriptStyle"
						:subscriptIcon="g_subscriptIcon"
						:subscriptBool="g_subscriptBool"
						:buyBtnStyle="g_buyBtnStyle"
						:displayStyle="g_displayStyle"
						:productData="g_productData"
					></group-buy>
				</view>
				<!-- 团购 -->
				...
				<view v-if="item.id == 'goods'" style="background: #FFFFFF;padding: 0rpx 20rpx 0;border-radius: 10rpx;">
					<commodity
						...
						:showGoodsOriginalPrice="item.data.showGoodsOriginalPrice"
						:showGoodsPrice="item.data.showGoodsPrice"
						:showGoodsLevelPrice="item.data.showGoodsLevelPrice"
						:originalPriceLabel="item.data.originalPriceLabel"
						:priceLabel="item.data.priceLabel"
						:levelPriceLabel="item.data.levelPriceLabel"
						:showBuyBtn="item.data.showBuyBtn"
						:subscriptIcon="item.data.goodsTagPicUrl"
						:showGoodsTag="item.data.showGoodsTag"
						:buyBtnStyle="item.data.buyBtnStyle"
						:buyBtns="item.data.buyBtn"
						:buyBtnText="item.data.buyBtnText"
						:displayStyle="item.data.goodsStyle"
						:productData="item.data.list"
						:buyBtnPic="item.data.buyBtnPic"
						:buttonColor="item.data.buttonColor"
					></commodity>
				</view>
				<!-- 商品 -->
				...
				<view class="ceiling-box" v-if="item.id == 'label-bar'" style="padding-bottom: 20rpx;">
					<ceiling
						:list="item.data.label_list"
						@click="ceilingSwitch"
						:page="t_page"
						:styleBool="styleBool"
						:searchHeight="is_fixed == 1 ? 0 : searchHeight"
	...
	methods: {
		...
		getConfigData() {
			this.$http
				...
				.then(res => {
					if (res.code === 0) {
						let { cat_style, copyright, mall_setting, navbar, page_title } = res.data;
