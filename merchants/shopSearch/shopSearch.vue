	...
	<view class="container">
		<view class="tui-searchbox">
			<view class="tui-search-input">
				...
				<!-- #endif -->
				<input confirm-type="search" placeholder="请输入店铺名" :focus="true" auto-focus placeholder-class="tui-input-plholder"
	...
	export default {
		...
		methods: {
			navTo(name){  //历史搜索
				...
				uni.navigateTo({
					url:'../citywide/citywide?keywords='+name
				})
			},
			search(){ //输入搜索
				if(!this.key){
				...
				uni.setStorageSync('current',1)
				uni.navigateTo({
					url:'../citywide/citywide?keywords='+this.key
				})
			},
			back: function() {
				this.navBack();
