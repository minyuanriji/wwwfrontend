<template>
	<view class="jx-numberbox-class jx-numberbox">
		<view class="jx-numbox-icon jx-icon-reduce " :class="[disabled || min>=value?'jx-disabled':'']" @tap="reduce"
		 :style="{color:iconColor,fontSize:iconSize+'rpx'}"></view>
		<input type="number" v-model="inputValue" :disabled="disabled" @blur="blur" class="jx-num-input" :style="{color:color,fontSize:size+'rpx',background:bgcolor,height:height+'rpx',width:width+'rpx'}" />
		<view class="jx-numbox-icon jx-icon-plus" :class="[disabled || value>=max?'jx-disabled':'']" @tap="plus" :style="{color:iconColor,fontSize:iconSize+'rpx'}"></view>
	</view>
</template>
