			...
			<view class="content">
				...
				<view class="content-head">
					...
					<view class="super" v-if="info">
						...
						<block v-if="info.is_parent==1">
							<image :src="info.parent_agent.avatar_url" mode="aspectFill" class="acatar"></image>
						</block>
				...
				<view class="content-center">
					...
					<view class="main-content flex flex-x-between flex-y-cente">
						<scroll-view :scroll-x="true" class="main-scroll">
							<view class="main-item" v-for="(item,index) in level_list" :key='index'>
								<view class="people" :style="{color:textColor}">{{item.total}}人</view>
				...
				<view class="content-bottom">
					<view class="order">
						...
						<view class="type2-box flex" v-if="tabIndex == 0">
							<view class="type2-item"  @tap="switchType2(index)" v-for="(item,index) in type2_list" :key='item'>
								<text class="type2-text" :style="{'border-bottom':index == type2_index?'2px solid'+textColor:''}">
									{{item}}
							...
							<view class="item" v-for="(item, i) in list" :key="i">
								<view class="user-status">
									...
									<view class="name-datetime">
										...
										<view class="tel">
											{{ item.mobile }}
											<span class="iconfont icon-dianhua3" :style="{color:textColor}" style="margin-left: 10rpx;"></span>
										</view>
					...
					.then(res => {
						...
						if (res.code == 0) {
							if (this.page == 1) {
								...
								this.team_level_list = JSON.parse(JSON.stringify(this.level_list));
								this.team_level_list.unshift({
									level:0,
									name:'全部'
								})
							} else {
