			...
			<view class="content">
				...
				<view class="content-bottom">
					<view class="card">
						...
						<view class="bill">
							<view class="icon-text flex-column-x-center" v-if="info&&info.level_list.length>0" v-for="(item,index) in info.level_list">
					...
					<view class="order">
						<view class="tabs">
							<view class="tab" v-for="(tab,i) in team_level_list" :key="i" :style="{color:tabIndex == i ?textColor:''}" @click="switchTab(i,tab.level)">
								<span class="name" :style="{'border-bottom':tabIndex == i ?'1px solid'+textColor:''}">{{tab.name}}</span>
							</view>
						</view>
